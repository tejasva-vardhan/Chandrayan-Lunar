"""EXP-002 orchestration tests on synthetic products.

These run both scale-policy variants without the external dataset.
...
from src.io.exp002.run import Exp002Error, run_exp002, run_exp002_from_products
...
def test_both_variants_run_and_record_required_metrics(synthetic_products, tmp_path: Path) -> None:
    source, reference = synthetic_products
    record = run_exp002_from_products(
        source,
        reference,
...
def test_variant_a_uses_the_frozen_representation_path(synthetic_products, tmp_path: Path) -> None:
    source, reference = synthetic_products
    record = run_exp002_from_products(
    ...
    assert variant_b["scale_policy"] == SCALE_POLICY_COMMON_PHYSICAL_GSD
    assert variant_b["generate_representation"]["source_matching_view"]["stride"] == 2
...
def test_comparison_table_has_one_row_per_variant(synthetic_products, tmp_path: Path) -> None:
    ...
    for row in rows:
        ...
        assert "effective_gsd_source" in row
...
def test_missing_data_root_is_reported_not_guessed(
    ...
    with pytest.raises(Exp002Error, match=DATA_ROOT_ENV):
        run_exp002(data_root=None, output_dir=tmp_path, record_path=tmp_path / "x.json")
