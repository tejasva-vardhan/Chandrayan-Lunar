...
def test_real_lroc_demo_products_when_available() -> None:
    root = os.environ.get(DATA_ROOT_ENV) or os.environ.get("LUNAR_DEMO_DATA_ROOT")
    if not root:
        pytest.skip(f"set {DATA_ROOT_ENV} to enable optional real-data ingestion checks")

    data_root = Path(root)
    available = _discover_expected_products(data_root)
    if not available:
        pytest.skip(f"no demo LROC PDS3 products found under {DATA_ROOT_ENV}")
