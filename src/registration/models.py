"""Replaceable 2D maps for baseline registration.

Same algebraic families as verification's software baselines (normalized DLT
projective map; affine least squares). Reimplemented here so registration does
not import verification internals.

None of these is the selected lunar transformation (D-010). Terrain relief,
pushbroom, DEM, RPC, and polynomial families are not implemented.
"""
...
# Linear-algebra floors. Engineering safeguards, not inlier thresholds.
_RANK_FLOOR = 1e-10
_MATRIX_NORM_FLOOR = 1e-12


class TransformModel(ABC):
    """Maps source_xy to reference_xy. Software baseline abstraction."""
...
class Affine2DBaseline(TransformModel):
    """Six-parameter affine map. SOFTWARE BASELINE, not a lunar model."""
...
class Projective2DBaseline(TransformModel):
    """Eight-dof 2D projective map via normalized DLT.

    SOFTWARE BASELINE, consistent with verification's projective_2d_baseline
    so synthetic pipelines share a coordinate convention. Not a universal
    lunar model (D-010).
    """
...
def get_transform_model(model_id: str) -> TransformModel:
    registry: dict[str, type[TransformModel]] = {
    ...
    try:
        ...
        raise ValueError(f"unknown registration model_id={model_id!r}; known: {known}") from exc
    return cls()
